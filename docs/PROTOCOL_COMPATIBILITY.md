...
| `/v1/chat/completions` | no | v0.2 |
| `/v1/responses` | no | v0.2 |
...
| `temperature` / `top_p` / `stop_sequences` / `tool_choice` | advisory / unsupported | Accepted as unknown request fields (not rejected) but **not mapped** to `@cursor/sdk`. v0.1 does not claim equivalent sampling or tool-choice behavior. |
