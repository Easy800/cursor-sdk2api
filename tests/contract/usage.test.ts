...
test("usage is deferred on tool turns and confirmed once on the final turn", async () => {
  ...
  expect(toolTurn.usage.output_tokens).toBe(0);
  expect(toolTurn.usage.cache_creation_input_tokens).toBeUndefined();
